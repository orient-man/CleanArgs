...
type Schema = (char * SchemaElement) list
type ErrorCode = | InvalidArgumentName of char
type SchemeParsingResult = Result<Schema, ErrorCode>

let parseSchema (schema: string) =
    let rec parseList acc list =
        match list with
        | [] -> Success(acc)
        | (name, param)::tail when Char.IsLetter name -> parseList ((name, param)::acc) tail
    ...
    |> List.ofSeq
    |> parseList []
    |> map (List.map (fun (name, _) -> (name, Bool)))
